...
def main():
        ...
        if file_type == "Image":
            ...
            if st.button("Extract Text from Image"):
                ...
                if ocr_method == "Method 1":
                    st.session_state.notes = extract_text_from_image('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', image_to_process)
    ...
    if st.session_state.notes:
        ...
        if "edit_mode" in st.session_state and st.session_state.edit_mode:
            edited_notes = st.text_area("Edit your notes here:", st.session_state.notes)
    ...
    if st.session_state.use_case_description:
        ...
        st.session_state.use_case_description = st.text_area("Edit Use Case Description:", st.session_state.use_case_description)
    ...
    if st.session_state.use_case_finalized:
        ...
        if st.session_state.current_item_index < len(keys):
            ...
            st.write(f" AI Opnion for {current_key}: /n {opion_ai}")

            action = st.selectbox(f"Select action for {current_key}:", ["Accept", "Edit", "Add"])

            if action == "Accept" and st.button(f"Confirm {current_key}"):
                st.session_state.current_item_index += 1
                st.success(f"{current_key} accepted!")

            elif action == "Edit":
                edited_value = st.text_input(f"Edit {current_key}:", st.session_state.items_dict[current_key])
                if st.button(f"Save {current_key}"):
                ...
                additional_value = st.text_input(f"Add to {current_key}:")
                if st.button(f"Add to {current_key}"):
        ...
        if st.session_state.current_item_index >= len(keys):
            ...
            docx_buffer = create_document_2(r"C:\Users\cem\Desktop\cekya_ıs\s\proposal_droid\Screenshot 2024-05-23 171557.png", st.session_state.items_dict)
            st.download_button(
                label="Download DOCX",
                data=docx_buffer,
